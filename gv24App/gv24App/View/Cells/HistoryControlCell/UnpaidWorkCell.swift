...
import Foundation
import UIKit
class UnpaidWorkCell: BaseCollectionCell{
    ...
    var taskInfo: TaskInfo?{
        didSet{
            labelTitle.text = taskInfo?.title
            iconType.loadImageurl(link: (taskInfo?.work?.image)!)
            labelDate.text = Date(isoDateString: (taskInfo?.time?.startAt)!).dayMonthYear
            labelTimes.text = Date(isoDateString: (taskInfo?.time?.startAt)!).hourMinute + " - " + Date(isoDateString: (taskInfo?.time?.endAt)!).hourMinute
        }
    }
}
